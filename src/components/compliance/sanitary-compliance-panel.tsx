    ...
    <div className="space-y-6">
      <div className="space-y-4 rounded-3xl border border-primary-100/60 bg-white/80 p-4 dark:border-white/10 dark:bg-white/5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-[var(--brand-muted)]">
              Tablero táctil para Baño, Cocina y Barra · registro automático de staff y horario.
            </p>
          </div>
          <label className="text-xs text-[var(--brand-muted)]">
            Mes
            <input
              type="month"
              value={hygieneMonth}
        ...
        {selectedArea !== 'BAÑO' && (
          <div className="space-y-3">
            <p className="text-sm text-[var(--brand-muted)]">
              Confirma limpieza profunda en {HYGIENE_AREAS[selectedArea].label}. Agrega notas para
              tareas específicas (trapeado, sanitización de superficies, restock de insumos).
            </p>
            <div className="grid gap-3 sm:grid-cols-2">
        ...
        <div className="space-y-2 text-xs">
          <p className="uppercase tracking-[0.3em] text-[var(--brand-muted)]">Últimos registros</p>
          {hygieneHistory.length === 0 ? (
            <p className="text-[var(--brand-muted)]">Aún no hay registros este mes.</p>
          ) : (
          ...
          <div className="rounded-2xl bg-white/60 p-3 text-sm text-amber-900 dark:bg-amber-900/30 dark:text-amber-50">
            ...
            {pestStatus?.latest ? (
              <>
                ...
                {pestStatus.latest.providerName && (
                  <p className="text-xs">Proveedor {pestStatus.latest.providerName}</p>
                )}
                ...
                {pestStatus.latest.nextServiceDate && (
                  <p className="text-xs">
                    Próximo servicio:{' '}
                    {new Date(pestStatus.latest.nextServiceDate).toLocaleDateString('es-MX')}
                  </p>
                )}
                {pestStatus.latest.observations && (
                  <p className="text-xs text-amber-700 dark:text-amber-200">
                    Observaciones: {pestStatus.latest.observations}
                  </p>
                )}
              </>
            ) : (
          ...
          <div className="space-y-2">
            <label className="block text-xs">
              ...
              <input
                ...
                onChange={(event) =>
                  setPestForm((prev) => ({ ...prev, providerName: event.target.value }))
                }
            ...
            <label className="block text-xs">
              ...
              <input
                ...
                onChange={(event) =>
                  setPestForm((prev) => ({ ...prev, certificateNumber: event.target.value }))
                }
                className="mt-1 w-full rounded-2xl border border-amber-200 bg-white/90 px-3 py-2 text-amber-900 focus:border-amber-400 focus:outline-none"
              />
            </label>
            <label className="block text-xs">
              ...
              <input
                ...
                onChange={(event) =>
                  setPestForm((prev) => ({ ...prev, serviceDate: event.target.value }))
                }
                className="mt-1 w-full rounded-2xl border border-amber-200 bg-white/90 px-3 py-2 text-amber-900 focus:border-amber-400 focus:outline-none"
              />
            </label>
            <label className="block text-xs">
              ...
              <input
                ...
                onChange={(event) =>
                  setPestForm((prev) => ({ ...prev, nextServiceDate: event.target.value }))
                }
                className="mt-1 w-full rounded-2xl border border-amber-200 bg-white/90 px-3 py-2 text-amber-900 focus:border-amber-400 focus:outline-none"
              />
            </label>
            <label className="block text-xs">
              ...
              <textarea
                ...
                onChange={(event) =>
                  setPestForm((prev) => ({ ...prev, observations: event.target.value }))
                }
                className="mt-1 w-full rounded-2xl border border-amber-200 bg-white/90 px-3 py-2 text-amber-900 focus:border-amber-400 focus:outline-none"
              />
            </label>
            ...
            {lowStockEntries.length === 0 ? (
              ...
              <div className="mt-2 space-y-2">
                {lowStockEntries.map((entry: SmartInventoryEntry) => (
                  <div key={`${entry.branchId}-${entry.itemId}`} className="rounded-2xl border border-emerald-100/70 px-3 py-2 text-xs text-emerald-900 dark:border-emerald-300/30 dark:text-emerald-50">
                    <p className="font-semibold">{entry.name}</p>
                    <p>
                      Stock {entry.quantity.toFixed(1)} {entry.unit} · mínimo{' '}
                      {entry.minStock.toFixed(1)} ({Math.round(entry.percentAvailable * 100)}%)
                    </p>
      ...
      <div className="space-y-4 rounded-3xl border border-slate-200 bg-white/90 p-4 text-sm text-[var(--brand-text)] dark:border-white/10 dark:bg-white/5 dark:text-white">
        ...
        <div className="space-y-2 text-xs">
          ...
          {wasteLogs.length === 0 ? (
            <p className="text-[var(--brand-muted)]">
              Completa el cierre sanitario antes del corte de caja.
            </p>
          ) : (
            wasteLogs.slice(0, 4).map((log: WasteLogEntry) => (
              <div key={log.id} className="rounded-2xl border border-slate-100 px-3 py-2 text-[var(--brand-text)] dark:border-white/10">
                ...
                <p>
                  Orgánico bebida {log.organicBeveragesKg}kg · Alimentos {log.organicFoodsKg}kg · Inorgánico{' '}
                  {log.inorganicKg}kg
                </p>
              </div>
            ))
